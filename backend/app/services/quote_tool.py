...
ARCHITECTURE UPDATES:
  - Parallel fetching: all scrapers run concurrently via ThreadPoolExecutor,
    so total quote time = max(scraper) instead of sum(scraper).
  ...
  - Mock fallback only fires when ALL scrapers return nothing — we never
    mix mock quotes with live ones so the agent always reasons on real data
    when any scraper succeeds.
  - Delhivery & Shiprocket: attempted but documented as unreliable (login-gated /
    test-pincode-rejecting). They return [] gracefully.
"""
import re
import time
...
PAGE_TIMEOUT_MS = 15000  # 15s per scraper — parallel fetch means total wall time is ~15s max
SCRAPER_GOTO_TIMEOUT_MS = 12000  # page.goto timeout — if the page itself doesn't load in 12s, bail
SCRAPER_RESULT_TIMEOUT_MS = 12000  # wait_for_selector for result elements — if results don't appear in 12s, bail

# These calculators mark Shipment Value as a required field, so we default
# to a modest prepaid shipment. (The selection agent DOES get package_value separately.)
...
def get_mock_quotes(origin: str, destination: str, weight_kg: float) -> list[dict]:
    ...
    Only used when ALL scrapers fail — never mixed with live quotes."""
...
def get_wareiq_quotes(origin: str, destination: str, weight_kg: float) -> list[dict]:
    """wareiq.com multi-courier calculator. Results appear in a modal overlay
    after clicking Calculate. We wait for the modal to become visible, then
    give the table rows time to stream in before reading them."""
    ...
    try:
        with sync_playwright() as p:
            ...
            # Results render in a modal (#ratesModal). Wait for it to appear,
            # then give the table rows time to stream in. The table-body tr
            # selector may exist from a stale render, so we wait for the modal
            # to be visible AND then wait for actual row content.
            page.wait_for_selector("#ratesModal", state="visible", timeout=SCRAPER_RESULT_TIMEOUT_MS)
            page.wait_for_timeout(3000)  # let rows stream in after modal opens
...
def get_nimbuspost_quotes(origin: str, destination: str, weight_kg: float) -> list[dict]:
    """nimbuspost.com shipping-rate-calculator. The page structure has been
    updated — the calculate button no longer has #calculate-btn, and results
    render into a table that appears after the API call completes."""
    ...
    try:
        with sync_playwright() as p:
            ...
            page.locator("#shipping-pickup-pincode").fill(origin)
            page.locator("#shipping-delivery-pincode").fill(destination)
            page.locator("#shipping-weight").fill(str(weight_kg))
            page.locator("#option1").check()  # Prepaid
            page.locator("#shipmentValue").fill(DEFAULT_SHIPMENT_VALUE)

            # The calculate button is now identified by text, not by ID
            page.locator("button:has-text('Calculate Now')").click()

            # Results appear in a table after the API call. Wait for the
            # table tbody to have rows with actual content.
            page.wait_for_selector("table tbody tr", timeout=SCRAPER_RESULT_TIMEOUT_MS)
            page.wait_for_timeout(2000)  # let all rows render
            ...
            for row in page.locator("table tbody tr").all():
                ...
                if not name:
                    continue
                # Price is typically in the 2nd or 3rd column; EDD in the next
                price_idx = 1
                for idx in range(1, min(len(cells), 4)):
                    txt = cells[idx].inner_text()
                    if "₹" in txt or "Rs" in txt:
                        price_idx = idx
                        break
                price = _price_from_text(cells[price_idx].inner_text())
                eta_idx = price_idx + 1 if price_idx + 1 < len(cells) else price_idx
                eta_text = cells[eta_idx].inner_text() if eta_idx < len(cells) else ""
                if price <= 0:
                    continue
                results.append({
                    "fleet_name": name,
                    "price": price,
                    "eta_hours": _parse_edd_hours(eta_text),
...
def get_delhivery_quotes(origin: str, destination: str, weight_kg: float) -> list[dict]:
    ...
    try:
        with sync_playwright() as p:
            ...
            page.locator("input[name='cod'], #cod").first.check()
...
def get_borzo_quote(quick: dict, weight_kg: float) -> dict | None:
    ...
    ETA is estimated at 2-5 hours for intra-city 2-wheeler same-day delivery.
    """
    ...
    try:
        with sync_playwright() as p:
            ...
            if len(prices) < 5:
                ...
                return None
            buckets = [float(pr.replace(",", "")) for pr in prices[:5]]
            ...
            return {
                ...
                "price": price,
                "eta_hours": 4,  # same-day intra-city 2-wheeler (estimated)
                "source": "borzo_live",
                "category": "quick",
            }
    except Exception:
...
def get_porter_quote(city: str, weight_kg: float) -> dict | None:
    ...
    From ₹48". This is a *base fare* (1 km included), not a route quote.
    ETA is estimated at 1-4 hours for intra-city 2-wheeler delivery.
    """
    ...
    try:
        with sync_playwright() as p:
            ...
            return {
                ...
                "price": price,
                "eta_hours": 3,  # 2-wheeler intra-city (estimated, same-day)
                "source": "porter_live",
                "category": "quick",
            }
    except Exception:
...
def get_all_quotes(origin: str, destination: str, weight_kg: float, *, quick: dict | None = None) -> list[dict]:
    ...
    ThreadPoolExecutor, so total wait = max(scraper) not sum(scraper).

    Mock fallback only fires when ALL scrapers return nothing — we never mix
    mock quotes with live ones, so the agent always reasons on real data when
    any scraper succeeds. This is important: mixing mock DTDC with real Porter
    would give the agent a false sense of the competitive landscape.

    Long-haul (standard) quotes are always fetched from pincodes. Quick-commerce
    quotes are only fetched when `quick` is provided with a pickup_city.
    """
    live: list[dict] = []
    ...
    # Only fall back to mock quotes if EVERY scraper failed. When we have any
    # live data, return only that — the agent reasons on real prices, not a
    # mix of real and fake that could mislead the selection.
    if not live:
